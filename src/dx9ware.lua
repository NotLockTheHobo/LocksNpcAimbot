	...
	for a, b in next, v.GetChildren(loc) do
		...
		if head then
			...
			if (headwts.x > 0 or headwts.y > 0) and distance3 < settings["Aimbot"]["max_distance"] then
				if closest ~= nil then
					if
						com(closest.dis, closest.dis3) > com(distance2, distance3)
						and distance2 < settings["Aimbot"]["fov_size"]
					then
						closest = {
							model = b,
							pos = headpos,
				...
				if setesp["enabled"] then
					...
					if setesp["name"] then
						...
						v.DrawString(
							{ headwts.x + setesp["name_offset"]["x"], headwts.y + setesp["name_offset"]["y"] },
							setesp["name_color"],
							text
						)
					end
					...
					if setesp["distance"] then
						...
						v.DrawString(
							{ headwts.x + setesp["distance_offset"]["x"], headwts.y + setesp["distance_offset"]["y"] },
							setesp["distance_color"],
							text
						)
					end
...
if settings["Aimbot"]["enabled"] then
	...
	if settings["Aimbot"]["target_dot"] then
		v.DrawCircle(
			{ closest.pos2.x, closest.pos2.y },
			settings["Aimbot"]["target_dot_color"],
			settings["Aimbot"]["target_dot_size"]
		)
	end
end
